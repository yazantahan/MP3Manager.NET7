...
namespace MP3Manager.NET7
{
    public class MP3Engine
    {
        ...
        private List<String> subMP3Files;
        private List<String> listChildFolders;

        private List<string> initNewMP3Files()
        {
            subMP3Files = new List<string>();
            return subMP3Files;
        }
        ...
        public int getYearField(int index)
        {
            try
            {
                return (int) TagLib.File.Create(mp3Files.ElementAt(index - 1)).Tag.Year;
            }
            catch (ArgumentOutOfRangeException)
            {
                return 0;
        ...
        public string getLyricsField(int index)
        {
            try
            {
                return TagLib.File.Create(mp3Files.ElementAt(index - 1)).Tag.Lyrics;
            } catch (ArgumentOutOfRangeException)
            {
                return "";
            }
        ...
        public void editMP3Info(string artist, string title, string album, int index, bool getLyrics = false)
        {
            editMP3(artist, title, album, index - 1, getLyrics);
        }
        ...
        /// <param name="title">The name of the title that needs to be edited</param>
        /// <param name="index">The selected .mp3 file using an integer or number</param>
        public void editMP3Info(string artist, string title, int index, bool getLyrics = false)
        {
            editMP3(artist, title, "", index - 1, getLyrics);
        }
        ...
        public List<String> CollectSubFolders()
        {
            /*Initialize the list of a selected .mp3 files*/
            initNewMP3Files();

            /*Declare a list of child folders*/
            listChildFolders = new List<string>();
        ...
        public void addMP3Files()
        {
            ...
            foreach (var childFolder in listChildFolders)
            {
                ...
                if (Directory.GetFiles(childFolder, "*.mp3").Length != 0)
                {
                    ...
                    foreach (var mp3File in Directory.GetFiles(childFolder))
                    {
                        subMP3Files.Add(mp3File);
                    }
                }
            }
        }

        public void extractMP3FilesfromSubFolder(int index)
        {
            var mp3File = subMP3Files[index];
            fm.MoveFile(mp3File, mainDirectoryPath);
        }
        ...
        public int getMP3FilesSubFolders()
        {
            return subMP3Files.Count;
        }

        public string getMP3FileInfoSubFolder(int index)
        {
            return subMP3Files[index];
        }
        ...
        public string GetMP3FileDetails(int index)
        {
            if (validateMP3File(index) == FILE_EXISTS)
            {
                ...
                if (!string.IsNullOrEmpty(mp3File.Tag.Lyrics) || !string.IsNullOrWhiteSpace(mp3File.Tag.Lyrics))
                {
                    MP3Details += "Lyrics: " + mp3File.Tag.Lyrics + "\n";
                }
        ...
        private void editMP3(string artist, string title, string album, int index, bool getLyrics = false)
        {
            ...
            if (getLyrics)
                mp3.Tag.Lyrics = this.getLyrics(artist, title);

            mp3.Save();
        }

        /// <summary>
        /// This function is used to Get lyrics through The internet
        /// </summary>
        /// <param name="artist">the Artist of the .MP3 File</param>
        /// <param name="title">the Title of this .MP3 File</param>
        /// <returns>return empty if it's not exists and a lyric if exists</returns>
        private string getLyrics(string artist, string title)
        {
            ...
            var searchRequest = new ArtistAndSongSearchRequest(artist, title);
            SearchResult searchResult;

            try
            {
                searchResult = lyricsScraperClient.SearchLyric(searchRequest);
            } catch (MusixmatchRequestException e)
            {
                return "";
            }
